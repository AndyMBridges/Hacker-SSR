...
import createStore from '../dist_client/store'
import App from '../dist_client/App'
import { getAllPosts } from '../dist_client/actions/postsActions';
...
export default (req: any, res: any) => {
  ...
  */
  const injectHTML = (
    data: any,
    { html, title, meta, body, scripts, state, styles }: Props
  ) => {
  ...
  fs.readFile(
    ...
    async (err, htmlData) => {
      ...
      let store: any
      if (createStore && getAllPosts) {
        store = createStore(req.url).store

        // ASYNC REDUX ACTIONS CAN GO HERE
        await store.dispatch(getAllPosts('typescript'));
      ...
      frontloadServerRender(() =>
        ...
        if (context.url) {
          ...
          const html = injectHTML(htmlData, {
            ...
            styles: new ServerStyleSheet().getStyleTags()
          })
